...
TASK_FILE = DATA_DIR / "tasks_v3.json"
REPORT_FILE = DATA_DIR / "reports_v3.json"
...
status_probe = DeerFlowStatusProbe()
result_mapper = DeerFlowResultMapper()

app = FastAPI(title="AI Industry Report System V3", version="0.4.0")
...
def generate_report(task_id: str, topic: str, time_range: str):
    try:
        ...
        if result.get("mode") == "deerflow":
            ...
            html = mapped.get("html", "")
            report_mode = mapped.get("mode", "deerflow")
        else:
            markdown = result.get("markdown", "") or json.dumps(result, ensure_ascii=False, indent=2)
            html = result.get("html", "")
            report_mode = result.get("mode", mode_selector.current_mode())
...
@app.get("/")
def root():
    return {"msg": "AI Industry Report System V3 Running", "mode": mode_selector.current_mode()}


@app.get("/health")
def health():
    base = {"status": "ok", "mode": mode_selector.current_mode()}
    base.update(status_probe.inspect())
    return base


@app.get("/api/runtime/status")
def runtime_status():
    return status_probe.inspect()
